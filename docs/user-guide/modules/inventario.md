<article>
  <div class="page-footer">LogixOne · Manual de Inventario · edición 2026-08-11 · página <span class="page-number"></span></div>
  <header class="cover">
    ...
    <div class="meta"><strong>Versión documentada:</strong> plugin <code>inventory</code> 1.0, baseline Sprint 8.<br><strong>Audiencia:</strong> responsables de almacén, operadores de stock, supervisores y soporte.<br><strong>Dependencia funcional:</strong> Catálogo comercial, consultado por contrato; Inventario conserva snapshots y no accede a sus tablas privadas.</div>
  </header>
  ...
  <section class="screen" data-screen="inventory-stock">
    ...
    <div class="wireframe">┌ Inventario · Existencias ─────────────────────────────────────────┐
│ Buscar [código/nombre] Estado [▼] [Buscar]                         │
├ Artículos ────────────┬ Disponibilidad por dimensiones ────────────┤
│ artículo · tracking   │ almacén · ubicación · condición · lote/serie│
│ [Seleccionar]         │ físico · reservado · disponible · versión  │
├ Incorporar artículo del catálogo ──────────────────────────────────┤
│ artículo [▼] tracking [▼] caducidad [▼] [Incorporar]               │
├ Movimiento ─────────────────────┬ Reserva / operación de reserva ──┤
│ tipo, origen/destino, cantidad  │ dimensiones, cantidad, expiración│
│ razón, fuente, idempotencia     │ ID, versión [consumir/liberar]   │
└────────────────────────────────────────────────────────────────────┘</div>
    <h3>Datos y controles</h3>
    <table><thead><tr><th>Grupo</th><th>Datos explicados</th><th>Reglas</th></tr></thead><tbody>
      <tr><td>Búsqueda</td><td>Texto por código/nombre, estado activo/inactivo, tabla de resultados y Seleccionar.</td><td>Solo consulta; siempre dentro de la empresa.</td></tr>
      ...
      <tr><td>Movimiento</td><td>Tipo, ubicación origen/destino, condición, lote/serie/expiración, cantidad presentada, razón, tipo/ID de fuente e idempotencia.</td><td>Cantidad positiva. El tipo define dirección y ubicaciones requeridas. Contabilizar es una operación de negocio, no una edición de saldo.</td></tr>
      <tr><td>Reserva nueva</td><td>Artículo, ubicación, condición, lote/serie/expiración, cantidad, fecha de expiración, fuente e idempotencia.</td><td>No puede reservar más disponibilidad que la permitida. Fuente+idempotencia protegen reintentos.</td></tr>
      <tr><td>Operar reserva</td><td>ID de reserva, versión, cantidad e idempotencia; acciones Consumir, Liberar o Expirar.</td><td>Consumir reduce compromiso y normalmente acompaña salida; liberar devuelve disponibilidad; expirar aplica reglas temporales.</td></tr>
    ...
    <ol><li>Configure almacenes y ubicaciones.</li><li>Incorpore el artículo confirmando tracking y caducidad; estas decisiones afectan cada operación posterior.</li><li>Contabilice una entrada inicial con idempotencia única.</li><li>Revise disponibilidad por dimensiones.</li><li>Para comprometer stock, cree una reserva y conserve su ID/versión.</li><li>Consuma o libere la reserva usando una idempotencia nueva para cada operación lógica.</li></ol>
    <p class="warning"><strong>Errores frecuentes:</strong> ubicación de otro almacén, lote/serie requerido, expiración inválida, stock insuficiente, idempotencia reutilizada con datos diferentes, versión de saldo/reserva antigua, artículo o ubicación inactivos. No edite el saldo directamente ni reenvíe con otra clave sin entender el resultado anterior.</p>
...
└───────────────────────────────────────────────────────────────────┘</div>
  ...
  <section><h2>Accesibilidad, seguridad y soporte</h2><ul><li>Cantidades y estados siempre se expresan como texto, no solo color.</li><li>En compacto, formularios se apilan y las tablas adoptan presentación alternativa.</li><li>No comparta idempotencias entre operaciones distintas ni modifique stock con SQL.</li><li>Para soporte entregue empresa, artículo, almacén/ubicación, operación, idempotencia/correlación, hora y mensaje; no incluya secretos.</li></ul></section>
</article>
