# Runbooks

Procedimientos operativos para construir, desplegar, diagnosticar, respaldar, restaurar y recuperar el sistema.

Cada procedimiento debe declarar prerrequisitos, comandos, resultado esperado, validación y mecanismo seguro de reversión.

## Índice

- [Levantar Logixone con IntelliJ IDEA Ultimate 2026.2](levantar-logixone-intellij-idea-ultimate.md) ([PDF](../output/pdf/guia-levantar-logixone-intellij-idea-ultimate.pdf))
- [Levantar Logixone con Visual Studio Code](levantar-logixone-visual-studio-code.md)
- [Decisión y metodología del instalador Windows al cerrar cada Sprint](metodologia-instalador-windows-cierre-sprint.md)
- [Demo visual del instalador Windows interno — J11-S8-08](demo-instalador-windows-sprint-08.md)
- [Construcción local con Maven Wrapper](build-local.md)
- [Construcción de la imagen Docker](docker-build.md)
- [Validación y operación de Compose](compose.md)
- [Keycloak y OIDC para desarrollo y demo](keycloak-oidc.md)
- [Contexto empresarial confiable y autorización](trusted-context-authorization.md)
- [Shell Jakarta Faces y navegación autorizada](shell-ui.md)
- [Manual para presentar la demo visual de Sprint 3](manual-demo-visual-sprint-03.md)
- [Manual paso a paso de pruebas integrales J11-S4-08](manual-pruebas-j11-s4-08.md)
- [Demo técnica de `business_partners` — J11-S6-05](demo-business-partners-j11-s6-05.md)
- [Demo reproducible de `business_partners` — J11-S6-06](demo-business-partners-j11-s6-06.md)
- [Demo visual de cierre de Sprint 6 — J11-S6-07](demo-cierre-sprint-06.md)
- [Demo reproducible de plugins y `commercial_catalog` — J11-S7-06](demo-commercial-catalog-j11-s7-06.md)
- [Demo visual de cierre de Sprint 7 — J11-S7-07](demo-cierre-sprint-07.md)
- [Demo visual oficial de cierre técnico de Sprint 8 — J11-S8-07](demo-cierre-sprint-08.md)
- [Demo de datos de referencia normativos — J11-S8-C03](demo-datos-referencia-j11-s8-c03.md)
- [Construcción y operación del migrador](migrator.md)
- [Backup y restauración controlada de PostgreSQL](postgresql-backup-restore.md)
